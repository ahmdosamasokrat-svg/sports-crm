...
### One-Line Install Command (Ubuntu 24.04.4 Bare-Metal Fresh Installation)
Installs a clean system with default pipeline stages and roles, with zero demo customer data:
```bash
curl -sSL https://raw.githubusercontent.com/ahmdosamasokrat-svg/mpc-crm/main/install.sh | sudo bash
```

### One-Line Update Command (Update Existing System - Preserves All Data)
Updates an existing system (migrations, new stages/permissions, code, assets) while safely preserving all existing customer records and data:
```bash
curl -sSL https://raw.githubusercontent.com/ahmdosamasokrat-svg/mpc-crm/main/update.sh | sudo bash
```
